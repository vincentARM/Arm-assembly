...
/* modèle B 512MO   */
/* Test de variation de la luminosité d'une LED  */
/*********************************************/
...
.equ    NUMPIN,  18        @ N° du pin utilisé (18 correspond à la broche physique 12)
...
main:             /* programme principal */
	...
	mov r1,#NUMPIN
	mov r2,#10      @ 100% de lumiere
	mov r3,#5       @ pendant 5 secondes
    bl varLumiere
...
/***************************************************/
/* r0 adresse GPIO memoire  */
/* r1 N° du pin de la LED  */
/* r2 %  de 1 à 100   */
/* r3 durée de la variation */
varLumiere:
	push {fp,lr}    /* save des  2 registres frame et retour */
	//mov fp,sp      /* pointeur de frame */
	...
	//mov r1,r2
	//bl division
	//mov r4,r2       @ temps d'extinction
	mov r6,#101
	sub r6,r2     @ calcul difference entre 101 et le % demandé
	ldr r5,iFacteur
	mul r4,r6,r5     @ temps d'extinction
	mul r5,r2,r5    @ temps d'allumage
	
	
	ldr r0,[fp]   @ recup de r0 sur la pile 
	ldr r1,[fp,#4]   @ recup de r1 sur la pile 
	ldr r3,[fp,#12]   @ recup de r3 sur la pile 
	ldr r6,iFacteur1
	mul r3,r6,r3
	 bl vidtousregistres
	  /* Gestion des pins du GPIO avec r0 et r1 d'entrée */ 
...
1:	 /* boucle extinction allumage */
	mov     r0, r8          @ Adresse zones mémoire du map GPIO
    mov     r1,r6          @ N° du pin à utiliser 
    bl      clearPin      @ Appel extinction
	mov r0,#0     @ 0 secondes
	mov r1,r4  @  delai d'extinction
    bl attente
    mov     r0, r8          @ Adresse zones mémoire du map GPIO
    mov     r1,r6         @ N° du pin à utiliser  
    bl      setPin      @ Appel allumage
	mov r0,#0     @ 0 secondes
	mov	r1,r5  @ delai d'allumage
    bl attente
	subs r3,#1
	bgt 1b
...
100:    /* fin standard de la fonction  */
    ...
    bx lr                   /* retour de la fonction en utilisant lr  */		
iFacteur: .int 100000	
iFacteur1: .int 8   @ delai pour une seconde approx 	
