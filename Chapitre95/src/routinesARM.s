...
sZoneBin:       .space 36,' '
                .asciz "\n"
.align 4
ptzZoneHeap:    .int heap_begin
/***********************************************/
/* Données non initialisées                    */
/***********************************************/
.bss
.align 4
ptTableOccTas:     .skip 4
dwDebut:           .skip 8
dwFin:             .skip 8
...
.global conversionAtoD,debutChrono,stopChrono,affichetat
.global genereraleas,concatChaine,divisionS,comparaison,affregistres,reserverPlace,libererPlace
.global razLibel,strInsert,affichageReg2,strInsertAtChar,statsTas,verifierTableTas
/*******************************************/
...
/***********************************************/
conversiondeb:               @ INFO: conversiondeb
    push {fp,lr}    /* save des  2 registres */
    add fp,sp,#8    /* fp <- adresse début */
                    /*ici save autres registres */
    push {r0-r5}
...
affichageMess:            @ INFO: affichageMess
    push {r0-r2,r7,lr}    @ save des  registres
...
2:                        @ donc ici r2 contient la longueur du message
    ...
    svc #0                @ appel systeme
    pop {r0-r2,r7,pc}     @ restaur des  2 registres
...
/***************************************************/
/*   reserve place sur le tas               */
/***************************************************/
// r0 taille en octet à reserver
reserverPlace:         @ fonction
    push {r1-r4,lr}        @ save des registres
    mov r4,r0
    ldr r1,iAdrptTableOccTas
    ldr r1,[r1] 
    cmp r1,#0
    beq 4f
1:
    ldr r2,[r1,#Tas_taille]
    and r3,r2,#0xFFFFFF00
    //vidregtit recherche1
    lsr r3,r3,#8
    //vidregtit recherche
    cmp r4,r3              @ plus grand que la zone lue ?
    bgt 3f
    and r3,r2,#0xFF
    //vidregtit libre
    cmp r3,#0
    bne 3f                       @ zone occupée ?
    mov r2,#1                    @ top occupé 
    strb r2,[r1,#Tas_taille]     @ Attention stockage de l'octet au début petit bout
    ldr r0,[r1,#Tas_adresse]
    mov r3,#0                          @ raz zone
    mov r2,#0
2:
    strb r2,[r0,r3]
    ...
    b 100f
3:
    ldr r1,[r1,#Tas_suivant]
    cmp r1,#0
    bne 1b
    
4:                          @ nouvelle reservation sur le tas
    mov r1,r4
...
iAdrptTableOccTas:     .int ptTableOccTas
iAdrptzZoneHeap:       .int ptzZoneHeap
iAdrzEndZoneHeap:      .int heap_end
iAdrzZoneHeap:         .int heap_begin
szMessErrTas:          .asciz "\033[31mErreur : Tas trop petit !!!\033[0m \n"
.align 4

/******************************************************************/
/*   libération de la place sur le tas                         */ 
/******************************************************************/
// r0 contient l'adresse
// r1 contient la taille
libererPlace:                    @ INFO: statsTas
    push {r2-r7,lr}              @ save  registres
    //vidregtit liberer
    mov r4,r0                    @ save entrée
    ldr r2,iAdrptTableOccTas     @ adresse table
    ldr r3,[r2] 
    cmp r3,#0                    @ table vide ?
    bne 1f
    mov r0,#Tas_fin              @ reserve la place pour un poste de la table
    bl reserverPlace
    cmp r0,#-1
    beq 100f
    str r4,[r0,#Tas_adresse]
    lsl r1,r1,#8
    str r1,[r0,#Tas_taille]
    //vidregtit stock1poste
    mov r1,#0
    //str r1,[r0,#Tas_libre]
    str r1,[r0,#Tas_suivant]
    str r0,[r2]                  @ stocke adresse premier poste
    mov r0,#0
    b 100f
1:
    ldr r2,[r3,#Tas_taille]
    and r5,r2,#0xFFFFFF00
    lsr r5,r5,#8
    cmp r1,r5                    @ plus grand que la zone lue
    bgt 2f
    ldr r5,[r3,#Tas_adresse]
    cmp r5,r4
    bne 2f                      @ pas la bonne adresse
    mov r1,#0                   @ TODO: peut être verifier si adresse non libre
    strb r1,[r3,#Tas_taille]    @ stockage de l'octet top libre en début
    mov r0,#0
    b 100f
2:
    mov r7,r3
    ldr r3,[r3,#Tas_suivant]
    cmp r3,#0
    bne 1b
    mov r0,#Tas_fin              @ reserve la place pour un poste de la table
    bl reserverPlace
    ...
    //str r1,[r0,#Tas_libre]
    str r1,[r0,#Tas_suivant]
    str r0,[r7,#Tas_suivant]       @ stocke l'adresse dans noeud précédent
    mov r0,#0

100:
    pop {r2-r7,pc}                          @ restaur registers  
...
/* r0 return the address of new string  on the heap */
/* or -1 if error   */
strInsertAtChar:                 @ INFO: strInsertAtChar
    push {r1-r7,lr}                         @ save  registres
    mov r3,#0                                // length counter 
1:                                           // compute length of string 1
    ldrb r4,[r0,r3]
...
/*   Stats sur le tas                         */ 
/******************************************************************/
statsTas:                    @ INFO: statsTas
    push {r0-r7,lr}          @ save  registres
    ldr r1,iAdrzZoneHeap
    //ldr r1,[r1]
    ldr r0,iAdrbuffer_conversion
    mov r2,#16
    push {r0,r1,r2}    
    bl conversion
    ldr r0,iAdrbuffer_conversion
    ...
    bl retourligne
100:
    pop {r0-r7,pc}                          @ restaur registers  
iAdrbuffer_conversion:  .int buffer_conversion
/******************************************************************/
/*   Stats sur le tas                         */ 
/******************************************************************/
verifierTableTas:                    @ INFO: verifierTableTas
    push {r0-r7,lr}                  @ save  registres
    ldr r2,iAdrptTableOccTas     @ adresse table
    ldr r0,[r2] 
    cmp r0,#0                    @ table vide ?
...
1:
    add r3,r3,#1
    ldr r2,[r0,#Tas_taille]
    //vidregtit taille
    and r2,r2,#0xFF
    cmp r2,#1
    beq 2f
     vidmemtit poste r0 3 
    add r1,#1
2:
    ldr r0,[r0,#Tas_suivant]
    cmp r0,#0
    bne 1b
    vidregtit compteurR1etR3
    b 100f
99:
    vidregtit tablevide    
100:
