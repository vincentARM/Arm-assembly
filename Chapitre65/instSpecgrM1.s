...
/* métrologie de la conversion en ascci d'un nombre de 64 bits (double) */
...
divisionReg64U_1:
    ...
    mov r6,r0 
    mov r7,r1
    mov r2,#10            @ multiplication par 10 pour calculer le reste
    bl multiplicationReg64U
    subs r4,r0            @ calcul du reste 
    sbcs r5,r1
    mov r0,r6
    mov r1,r7
    mov r2,r4
    cmp r4,#10            @ correction si le reste est > 10
    blt 100f
    adds r0,#1            @ ajout 1 au quotient
    adc r1,#0            @ addition parties hautes avec retenue partie basse
    sub r2,#10            @ correction du reste
100:                   @ fin standard de la fonction
...
divisionReg64U_2:
    push {r3-r7,lr}      @ save des registres
    mov r4,r0            @ save nombre double déparrt 
    mov r5,r1
    lsr r6,r0,#1         @ decalage de 1  partie basse du nombre
    lsrs r7,r1,#1        @ puis decalage de 1 de la partie haute
    ...
    lsr r0,#1            @ et on recommence la partie basse du nombre
    lsrs r1,#1           @ puis on decale la partie haute
    orrcs r0,#0x80000000 @ et on pousse le bit de droite dans la partie basse
                         @ addition des 2 resultats précedents
    adds r6,r0           @ ajout partie basse
    adc r7,r1            @ addition parties hautes avec retenue partie basse
                         @ inutile de verifier l'overflow
    mov r0,r6
    mov r1,r7
                         @ decalage de 4 à droite par une autre méthode voir Hacker'Delight
    lsr r0,#4            @ décalage partie basse
    lsl r2,r1,#28        @ décalage gauche de 32 - 4 partie haute
    orr r0,r2            @ or partie basse
    mov r3,#-28          @ décalage droite de 4 - 32 (passage par le registre r3 car erreur si valeur immédiate)
    lsr r2,r1,r3         @ de la partie haute 
    orr r0,r2            @ et or de la partie basse
    lsr r1,#4            @ en enfin decalage à drouite de la partie haute
    adds r6,r0           @ ajout partie basse précédent résultat
    adcs r7,r1           @ addition parties hautes avec retenue partie basse
    movcs r6,#0          @ si carry mis -> overflow, raz du résultat partie basse
    movcs r7,#0          @ si carry mis -> overflow, raz du résultat partie haute
    ldrcs r0,iAdrszMessAddOver @ et message overflow
    ...
    mov r0,r6
    mov r1,r7
    lsr r0,#8            @ maintenant décalage de 8 à droite
    lsl r2,r1,#24
    orr r0,r2
    mov r3,#-24
    lsr r2,r1,r3
    ...
    adds r6,r0           @ ajout partie basse
    adc r7,r1            @ addition parties hautes avec retenue partie basse
                         @ inutile de vérifier l'overflow
    mov r0,r6
    mov r1,r7
    lsr r0,#16           @ maintenant décalage de 16 à droite
    lsl r2,r1,#16
    orr r0,r2
    mov r3,#-16
    lsr r2,r1,r3
    ...
    mov r0,r6
    mov r1,r7
    lsr r0,#32           @ maintenant décalage de 32 à droite
    mov r3,#32
    lsl r2,r1,r3
    orr r0,r2
    mov r3,#0
    ...
    mov r0,r6
    mov r1,r7
    lsr r0,#3            @ et finalement décalage de 3 à droite
    lsl r2,r1,#29
    orr r0,r2
    mov r3,#-29
    lsr r2,r1,r3
